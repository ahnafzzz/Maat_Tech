...
`deployment/tests/container-smoke-test.sh` provisions a fresh SQLite database as an explicit setup operation, creates synthetic persistent database and upload markers, and starts the built image with temporary storage and data volumes. It verifies `/up`, verifies no customer or administrator was seeded, restarts the container, and verifies both markers again. The volumes and container are removed afterward.

This smoke test demonstrates image construction, startup, and configured-volume persistence only. The current image starts Laravel with `php artisan serve`, which is a development server rather than a reviewed production HTTP runtime. The application also trusts forwarding headers from every address in production. Before hosting the container, select a production server/process model, restrict trusted proxies to the real ingress, verify persistent-volume behavior on the selected platform, and repeat database backup/restore testing with the production engine.
