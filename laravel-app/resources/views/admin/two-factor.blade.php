...
<body class="min-h-screen bg-[#050609] font-sans text-white">
    ...
    <main class="mx-auto flex min-h-screen max-w-md items-center px-5">
        <form method="POST" action="{{ route('admin.two-factor.verify') }}" autocomplete="off" class="w-full border border-tech-800 bg-cyber-panel/95 p-7 shadow-2xl shadow-teal-950/20 sm:p-9">
            @csrf
            ...
            @endif
            <label class="mt-6 block font-mono text-[10px] text-tech-300">VERIFICATION_CODE<input name="code" required inputmode="numeric" maxlength="6" autocomplete="one-time-code" class="mt-2 w-full border border-cyber-border bg-[#090d14] p-3 font-sans text-sm text-white outline-none focus:border-tech-500"></label>
