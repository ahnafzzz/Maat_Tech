...
<head>
    ...
    <title>Reset Password | MAAT TECHNOLOGIE BD</title>
...
<body class="min-h-screen bg-[#090b10] font-sans text-white">
    ...
    <main class="mx-auto flex min-h-screen max-w-md items-center px-5">
        <form method="POST" action="{{ route('password.update') }}" class="w-full border border-cyber-border bg-cyber-panel/90 p-7 shadow-2xl shadow-black/40 sm:p-9">
            @csrf
            ...
            <label class="mt-6 block font-mono text-[10px] text-slate-400">EMAIL_ADDRESS<input type="email" name="email" required value="{{ old('email', $email) }}" class="mt-2 w-full border border-cyber-border bg-[#090d14] p-3 font-sans text-sm outline-none focus:border-tech-500"></label>
            <label class="mt-4 block font-mono text-[10px] text-slate-400">NEW_PASSWORD<input type="password" name="password" required class="mt-2 w-full border border-cyber-border bg-[#090d14] p-3 font-sans text-sm outline-none focus:border-tech-500"></label>
            ...
            <button class="mt-6 w-full border border-tech-400 bg-tech-600 p-3 text-sm font-mono hover:bg-tech-500">RESET_PASSWORD</button>
        </form>
    </main>
</body>
</html>
